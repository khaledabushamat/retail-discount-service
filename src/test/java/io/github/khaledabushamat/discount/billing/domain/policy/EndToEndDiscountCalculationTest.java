...
public class EndToEndDiscountCalculationTest {
    ...
    void setUp() {
        ...
        engine = new DiscountEngine(
                List.of(
                        new EmployeeDiscountPolicy(rates),
    ...
    class WorkedExample {
        ...
        void employeeWithGroceriesAndElectronics() {
            var bill = bill(recentCustomer(CustomerType.EMPLOYEE), electronics("890.00"), groceries("100.00"));
    ...
    class SinglePercentageDiscount {
        ...
        void employeeAndAffiliateGetsTheHigherRate() {
            var bill = bill(recentCustomer(CustomerType.EMPLOYEE, CustomerType.AFFILIATE), electronics("1000.00"));

            assertThat(engine.calculate(bill).percentageDiscount()).isEqualTo(Money.of("300.00"));
        }

        @Test
        void longStandingAffiliateGetsAffiliateRateNotLoyalty() {
            var bill = bill(customer(LocalDate.of(2015, 1, 1), CustomerType.AFFILIATE), electronics("1000.00"));

            assertThat(engine.calculate(bill).percentageDiscount()).isEqualTo(Money.of("100.00"));
        }

        @Test
        void longStandingCustomerWithNoTypeGetsLoyaltyRate() {
            var bill = bill(customer(LocalDate.of(2015, 1, 1)), electronics("1000.00"));

            assertThat(engine.calculate(bill).percentageDiscount()).isEqualTo(Money.of("50.00"));
        }
    }
    ...
    class GroceryExclusion {
        ...
        void bulkDiscountStillCountsGroceriesTowardsTheThreshold() {
            var bill = bill(recentCustomer(), groceries("300.00"), electronics("50.00"));

            assertThat(engine.calculate(bill).flatDiscount()).isEqualTo(Money.of("15.00"));
        }
    }
    ...
    class BulkDiscount {
        ...
        void billAtExactlyOneHundredGetsFive() {
            ...
            assertThat(engine.calculate(bill).flatDiscount()).isEqualTo(Money.of("5.00"));
        }
    ...
    class LoyaltyBoundary {
        ...
        void exactlyTwoYearsDoesNotQualify() {
            var bill = bill(customer(BILLED_ON.minusYears(2)), electronics("1000.00"));

            assertThat(engine.calculate(bill).percentageDiscount()).isEqualTo(Money.ZERO);
        }

        @Test
        void twoYearsAndOneDayQualifies() {
            var bill = bill(customer(BILLED_ON.minusYears(2).minusDays(1)), electronics("1000.00"));
    ...
    class Combined {
        ...
        void newCustomerWithSmallBillGetsNothing() {
            ...
            assertThat(engine.calculate(bill).netPayable()).isEqualTo(Money.of("50.00"));
        }
