...
public class BillCalculationService {
    ...
    public BillCalculationService(
            CustomerRepository customers, ProductCatalog catalog, DiscountEngine engine, Clock clock) {
    ...
    public DiscountBreakdown calculate(String customerExternalId, List<LineItemRequest> lines) {
        ...
        List<LineItem> items =
                lines.stream().map(line -> toLineItem(line, products)).toList();
    ...
    private LineItem toLineItem(LineItemRequest line, Map<String, Product> products) {
        ...
        return new LineItem(product.id(), product.category(), product.unitPrice(), line.quantity());
    }
}
